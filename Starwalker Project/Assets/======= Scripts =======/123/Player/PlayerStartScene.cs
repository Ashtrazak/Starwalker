...
public class PlayerStartScene : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        _transform.position = new Vector3(_transform.position.x, StartValue, _transform.position.z); // Убирает рассинхронизацию значений при старте игры
    }
    void OnEnable()
    {
        GetComponent<PlayerLeaveScene>().enabled = false; // Отключаем скрипт противоположного действия
        _transform.position = new Vector3(0f, StartValue, transform.position.z);
    ...
    void Update()
    {
        ...
        _transform.position = new Vector3(_transform.position.x, curentValue, _transform.position.z);
    }
}
